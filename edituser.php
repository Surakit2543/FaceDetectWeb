<?php
    ...
    <div class="main2">
        <div class="bar">
        <i class="fas fa-table"></i>     ตาราง EDIT ข้อมูลสมาชิก
    ...
    <div class="row">
      ...
      <?php
		$sql = $conn->query("select * from student INNER JOIN subject ON student.sub_id=subject.sub_id where std_id = '$_REQUEST[id]'");
    $show = $sql->fetch_assoc();
		?>

          <div class="form-group col-md-6">
            <h3>รหัสนักศึกษา :</h3>
            <input name="stdid" type="text" class="form-control" required value="<?php echo $show['std_id'];?>">
          </div>

		      <div class="form-group col-md-6">
            <h3>ชื่อ :</h3>
            <input name="name" type="text" class="form-control" required value="<?php echo $show['std_name'];?>">
          </div>

          <div class="form-group col-md-6">
            <h3>อีเมล :</h3>
            <input name="email" type="email" class="form-control" required value="<?php echo $show['std_email'];?>">
          </div>

          <div class="form-group col-md-6">
            <h3>รายวิชา :</h3>
            <select id="subject" name="subject" class="form-control" onchange="subValue(this.value,<?php echo $show['std_id'];?>)">
...
if(isset($_GET['update'])){
    ...
    $sql = $conn->query("select * from student INNER JOIN subject ON student.sub_id=subject.sub_id where std_id = '$_REQUEST[stdid]' && std_id != '$_REQUEST[id]'")or die (mysqli_error());
    if($sql->num_rows>0){
    ...
    $sql = mysqli_query($conn,"update student INNER JOIN subject ON student.sub_id=subject.sub_id set std_id = '$_REQUEST[stdid]',
    student.std_name = '$_REQUEST[name]',std_email = '$_REQUEST[email]',std_class = '$_REQUEST[class]',std_field = '$_REQUEST[field]',
    std_section = '$_REQUEST[section]',student.sub_id = '$_REQUEST[subject]' where std_id = '$_REQUEST[stdid]'")or die (mysqli_error());
    
    //function check แก้ไขข้อมูล จะมี alert ขึ้นมา ตามเงื่อนไข
    Chk_Update($sql,'แก้ไขข้อมูลเรียบร้อย','user.php');
    }
...
<script language="javascript" type="text/javascript">
    function subValue(subid, stdid){
        document.location = 'edituser.php?subid=' + subid + '&id=' + stdid;
        //document.location = 'edituser.php?id=' + stdid;

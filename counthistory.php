...
<div class="vertical-menu">
    ...
    ?>
    <br><div class="table1">
    <center><div class="panel"><a href="counthistory.php"><input name="" type="button" class="btn btn-info" value="จำนวนผู้เข้าใช้งาน"></a></div></center>
    ...
    $sql = $conn->query("select count(h.std_id) c, h.std_id, std_name, login_date, logout_date, active_user from history h INNER JOIN 
    student s ON h.std_id=s.std_id group by h.std_id having c>1 order by h_id desc limit $start,$perpage")or die (mysqli_error());

    //หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกแสดงเพื่อจะเอาไปทำการแบ่งหน้า
    $sql2 = $conn->query("select count(h.std_id) c, h.std_id, std_name, login_date, logout_date, active_user from history h INNER JOIN 
    student s ON h.std_id=s.std_id group by h.std_id having c>1 order by h_id desc")or die (mysqli_error());
